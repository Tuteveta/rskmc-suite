...
export default function DonutChart({ data, centerLabel }: { data: Slice[]; centerLabel?: string }) {
    ...
    useEffect(() => {
        ...
        const slices = g
            .selectAll<SVGPathElement, d3.PieArcDatum<Slice>>('.slice')
            .data(pie(data))
            .join('path')
            .attr('class', 'slice')
        ...
        if (centerLabel) {
            g.append('text').attr('text-anchor', 'middle').attr('dy', '-0.4em').attr('font-size', 10).attr('fill', '#9ca3af').text(centerLabel);
        }
        g.append('text')
            ...
            <ul className="w-full space-y-2 text-sm">
                {data?.map((d, i) => (
                    <li key={d.label} className="flex items-center gap-2">
                        ...
                        <span className="w-10 text-right text-xs text-gray-400">{total > 0 ? `${((d.value / total) * 100).toFixed(0)}%` : '—'}</span>
                    </li>
                ))}
            </ul>
        </div>
