...
export default function BarRace({ data }: { data: RaceFrame[] }) {
    ...
    useEffect(() => {
        ...
        g.selectAll<SVGTextElement, (typeof entries)[0]>('.rval')
            .data(entries, (d) => d.label)
            .join(
                (enter) => enter.append('text').attr('class', 'rval').attr('x', 4).attr('font-size', 10).attr('fill', '#374151'),
                (update) => update,
            )
            .transition()
            .duration(dur)
            ...
            .text((d) => (d.value > 0 ? `PGK ${d.value.toLocaleString('en-PG', { maximumFractionDigits: 0 })}` : ''));
        ...
        svg.select<SVGGElement>('.x-axis')
            ...
            .call(
                d3
                    .axisBottom(x)
                    .ticks(5)
                    .tickFormat((v: d3.NumberValue) => (Number(v) >= 1000 ? `K${(Number(v) / 1000).toFixed(0)}k` : `${Number(v)}`)) as never,
            )
            .call((gg: AxisSelection) => gg.select('.domain').remove())
            .selectAll('text')
            .attr('font-size', 9)
        ...
        <div>
            ...
            <div className="mt-3 flex items-center justify-center gap-3">
                ...
                <button
                    ...
                    className="flex items-center gap-1.5 rounded-full bg-gray-900 px-4 py-1.5 text-xs font-medium text-white"
                >
                ...
                <span className="ml-1 w-16 text-xs text-gray-400 tabular-nums">{data[frame]?.month ?? ''}</span>
            </div>
        </div>
    );
}
