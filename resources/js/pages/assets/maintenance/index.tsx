...
export default function MaintenanceIndex({ asset, logs, types }: Props) {
    ...
    const destroy = (logId: number) => {
        if (confirm('Delete this log?')) router.delete(route('assets.maintenance.destroy', { asset: asset.id, log: logId }));
    };
        ...
        >
            ...
            <div className="space-y-4 p-4 sm:p-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <Link href="/assets" className="mb-1 flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
                            <ArrowLeft className="h-3.5 w-3.5" />
                            Back to Assets
                        </Link>
                        <h1 className="text-2xl font-semibold">Maintenance Log — {asset.name}</h1>
                    </div>
                    <Link href={route('assets.maintenance.create', asset.id)}>
                        <Button>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 text-xs text-gray-600 uppercase">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-right">Cost (PGK)</th>
                            ...
                            {logs.data.map((l) => (
                                <tr key={l.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3">
                                        <span
                                            className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${typeColor[l.type] ?? 'bg-gray-100 text-gray-600'}`}
                                        >
                                            {types[l.type]}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        {l.cost ? Number(l.cost).toLocaleString('en-PG', { minimumFractionDigits: 2 }) : '—'}
                                    </td>
                                    <td className="px-4 py-3">{l.next_due_date ?? '—'}</td>
                                    <td className="px-4 py-3">
                                        <div className="flex justify-end gap-2">
                                            <Link href={route('assets.maintenance.edit', { asset: asset.id, log: l.id })}>
                                                <Button size="sm" variant="outline">
                                                    Edit
                                                </Button>
                                            </Link>
                                            {canDelete && (
                                                <Button size="sm" variant="destructive" onClick={() => destroy(l.id)}>
                                                    Delete
                                                </Button>
                                            )}
                                        </div>
