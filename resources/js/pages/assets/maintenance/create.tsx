...
export default function MaintenanceCreate({ asset, types }: { asset: Asset; types: Record<string, string> }) {
    const { data, setData, post, processing, errors } = useForm({
        service_date: new Date().toISOString().split('T')[0], type: 'service',
        description: '', cost: '', performed_by: '', next_due_date: '', notes: '',
    });
    const submit = (e: React.FormEvent) => { e.preventDefault(); post(route('assets.maintenance.store', asset.id)); };

    return (
        <AppLayout breadcrumbs={[{ title: 'Assets', href: '/assets' }, { title: asset.name, href: '#' }, { title: 'Add Maintenance Log', href: '#' }]}>
                ...
                <form onSubmit={submit} className="space-y-4 glass rounded-xl p-6">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div><Label>Type *</Label>
                            <select className="w-full border rounded-md px-3 py-2 text-sm" value={data.type} onChange={e => setData('type', e.target.value)}>
                                {Object.entries(types).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                            </select>
                        </div>
                        <div className="col-span-2"><Label>Description *</Label><Input value={data.description} onChange={e => setData('description', e.target.value)} /><InputError message={errors.description} /></div>
