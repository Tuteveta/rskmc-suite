...
export default function MarriageRecordCreate({ members }: { members: Member[] }) {
    ...
    const submit = (e: React.FormEvent) => { e.preventDefault(); post(route('marriage-records.store')); };

    const MemberSelect = ({ label, field }: { label: string; field: 'husband_member_id' | 'wife_member_id' }) => (
        <div>
            <Label>{label} Member (optional)</Label>
            <select className="w-full border rounded-md px-3 py-2 text-sm" value={data[field]} onChange={e => setData(field, e.target.value)}>
            ...
            <div className="p-6 max-w-2xl">
                ...
                <form onSubmit={submit} className="space-y-5 glass rounded-xl p-6">
                    <div className="grid grid-cols-2 gap-4 border-b pb-4">
                        ...
                        <div />
                        <div><Label>First Name *</Label><Input value={data.husband_first_name} onChange={e => setData('husband_first_name', e.target.value)} /><InputError message={errors.husband_first_name} /></div>
                        <div><Label>Last Name *</Label><Input value={data.husband_last_name} onChange={e => setData('husband_last_name', e.target.value)} /><InputError message={errors.husband_last_name} /></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 border-b pb-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2"><Label>Witnesses</Label><Input value={data.witnesses} onChange={e => setData('witnesses', e.target.value)} placeholder="Comma-separated names" /></div>
