...
interface Record { id: number; member_id: number|null; first_name: string; last_name: string; date_of_birth: string|null; date_of_death: string; date_of_funeral: string|null; place_of_burial: string|null; officiant: string|null; cause_of_death: string|null; notes: string|null; }
...
export default function FuneralRecordEdit({ record, members }: { record: Record; members: Member[] }) {
    const { data, setData, put, processing } = useForm({
        ...
        date_of_birth: record.date_of_birth ?? '', date_of_death: record.date_of_death,
        date_of_funeral: record.date_of_funeral ?? '', place_of_burial: record.place_of_burial ?? '',
        officiant: record.officiant ?? '', cause_of_death: record.cause_of_death ?? '', notes: record.notes ?? '',
    });
    const submit = (e: React.FormEvent) => { e.preventDefault(); put(route('funeral-records.update', record.id)); };
            ...
            <div className="p-4 sm:p-6 w-full max-w-2xl">
                ...
                <form onSubmit={submit} className="space-y-4 glass rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div><Label>Date of Death *</Label><Input type="date" value={data.date_of_death} onChange={e => setData('date_of_death', e.target.value)} /></div>
