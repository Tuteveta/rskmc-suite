...
export default function TitheCreate({ members, giving_types }: { members: Member[]; giving_types: Record<string, string> }) {
    const { data, setData, post, processing, errors } = useForm({
        member_id: '', giving_type: 'tithe', amount: '', giving_date: new Date().toISOString().split('T')[0], service_type: '', notes: '',
    });

    const submit = (e: React.FormEvent) => { e.preventDefault(); post(route('tithes.store')); };

    return (
        <AppLayout breadcrumbs={[{ title: 'Tithes & Offerings', href: '/tithes' }, { title: 'Record Giving', href: '#' }]}>
            ...
            <div className="p-6 max-w-lg">
                ...
                <form onSubmit={submit} className="space-y-4 bg-white rounded-lg border p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <Label>Service</Label>
                            <Input placeholder="e.g. Sunday Morning" value={data.service_type} onChange={e => setData('service_type', e.target.value)} />
                        </div>
                    </div>
                    <div>
                        <Label>Notes</Label>
