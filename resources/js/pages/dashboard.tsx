...
interface PeriodVariances {
    new_members: Variance;
    baptisms: Variance;
    marriages: Variance;
    funerals: Variance;
    tithes: Variance;
...
function TrendBadge({ v, invert = false }: { v: Variance; invert?: boolean }) {
    const effectiveTrend = invert ? (v.trend === 'up' ? 'down' : v.trend === 'down' ? 'up' : 'neutral') : v.trend;
...
function VCard({
    ...
    const fmt = (n: number) => (isCurrency ? `PGK ${n.toLocaleString('en-PG', { minimumFractionDigits: 2 })}` : n.toLocaleString());
    ...
    return (
        <div className="glass flex flex-col gap-3 rounded-xl p-5">
            ...
            <div>
                ...
                <p className="mt-0.5 text-2xl font-bold text-gray-900">{fmt(variance.current)}</p>
                <p className="mt-0.5 text-xs text-gray-400">
                    ...
                    {variance.diff !== 0 && (
                        <span className={variance.diff > 0 ? 'text-green-600' : 'text-red-500'}>
                            {' '}
                            ({variance.diff > 0 ? '+' : ''}
                            {isCurrency ? `PGK ${variance.diff.toFixed(2)}` : variance.diff})
                        </span>
                    )}
                </p>
            </div>
...
interface FinancialAnalytics {
    ...
    unique_givers_month: number;
    unique_givers_year: number;
    avg_per_giver_year: number;
    monthly_avg_giving: number;
}
            ...
            <div className="flex flex-col gap-4 p-4 sm:gap-6 sm:p-6">
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-0.5 text-sm text-gray-500">{role} · RSKMC Suite</p>
                    </div>
                    {/* Period toggle */}
                    <div className="glass flex overflow-hidden rounded-lg text-sm">
                ...
                <p className="-mt-4 text-xs text-gray-400">
                    Showing variances <strong>{periodLabel[period]}</strong>
                </p>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                    ...
                    <VCard
                        label={`Giving (${period === 'year' ? 'Year' : period === 'month' ? 'Month' : 'Week'})`}
                        value={summary.tithes_month}
                        variance={v.tithes}
                        icon={DollarSign}
                        color="bg-green-500"
                        isCurrency
                ...
                <div className="grid gap-3 sm:grid-cols-4">
                    {[
                        ...
                        { label: 'Total Assets', value: summary.total_assets, sub: 'registered' },
                        {
                            ...
                            value: `PGK ${Number(summary.tithes_month).toLocaleString('en-PG', { minimumFractionDigits: 2 })}`,
                            sub: 'tithes & offerings',
                            raw: true,
                        },
                        {
                            label: 'Giving This Year',
                            value: `PGK ${Number(summary.tithes_year).toLocaleString('en-PG', { minimumFractionDigits: 2 })}`,
                            sub: 'cumulative',
                            raw: true,
                        },
                    ].map((s) => (
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <InsightCard
                        ...
                        value={`${member_analytics?.retention_rate ?? 0}%`}
                        sub="active members"
                    />
                    <InsightCard
                        icon={Clock}
                        ...
                        value={`${member_analytics?.avg_tenure_years ?? 0} yrs`}
                        sub="since joining"
                    />
                    <InsightCard
                        icon={Trophy}
                        iconBg="bg-amber-500"
                        label="Avg Giving / Member"
                        value={`PGK ${(financial_analytics?.avg_per_giver_year ?? 0).toLocaleString('en-PG', { minimumFractionDigits: 0 })}`}
                        sub="this year"
                    />
                    <InsightCard
                        icon={Wrench}
                        ...
                        value={asset_monitoring?.maintenance?.length ?? 0}
                        sub="assets"
                    />
                </div>
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    <ChartCard title="Member Insights">
                        <div className="space-y-5">
                            ...
                            {/* Gender split */}
                            <div>
                                <p className="mb-2 text-xs font-medium tracking-wide text-gray-500 uppercase">Gender Distribution</p>
                                        ...
                                        <div key={g.label} className="mb-2">
                                            <div className="mb-1 flex justify-between text-xs text-gray-600">
                                                ...
                                                <span className="font-semibold">
                                                    {g.value} <span className="text-gray-400">({pct}%)</span>
                                                </span>
                                            </div>
                    ...
                    <ChartCard title="Top Givers — This Year">
                        {(financial_analytics?.top_givers ?? []).length === 0 ? (
                            <p className="py-6 text-center text-sm text-gray-400">No giving records yet.</p>
                        ) : (
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-2 border-t border-gray-100 pt-2 text-xs text-gray-500">
                                    <div>
                                        <span className="font-semibold text-gray-700">{financial_analytics?.unique_givers_month ?? 0}</span> givers
                                        this month
                                    </div>
                                    <div>
                                        <span className="font-semibold text-gray-700">{financial_analytics?.unique_givers_year ?? 0}</span> givers
                                        this year
                                    </div>
                                </div>
                            </div>
                    ...
                    <ChartCard title="Asset Condition Overview">
                        <div className="space-y-4">
                            {(asset_monitoring?.by_condition ?? []).length === 0 ? (
                                <p className="py-6 text-center text-sm text-gray-400">No assets registered.</p>
                            ) : (
                                <>
                                    <div className="grid grid-cols-2 gap-2">
                                        {(asset_monitoring?.by_condition ?? []).map((c) => (
                                            <div key={c.label} className="flex items-center gap-2 rounded-lg border p-3">
                                                <div className="h-3 w-3 flex-shrink-0 rounded-full" style={{ background: c.color }} />
                                                <div>
                                    ...
                                    <div className="pt-1">
                                        <p className="mb-1.5 text-xs text-gray-500">Value distribution by condition</p>
                                        <ResponsiveContainer width="100%" height={120}>
                                            <BarChart
                                                data={asset_monitoring?.by_condition ?? []}
                                                layout="vertical"
                                                margin={{ left: 60, right: 40, top: 4, bottom: 4 }}
                                            >
                                                ...
                                                <Tooltip
                                                    formatter={(v: number) => `PGK ${v.toLocaleString('en-PG', { minimumFractionDigits: 0 })}`}
                                                />
                    ...
                    <div className="space-y-4">
                        <ChartCard title="Assets by Category">
                            {(asset_monitoring?.by_category ?? []).length === 0 ? (
                                <p className="py-4 text-center text-sm text-gray-400">No assets registered.</p>
                            ) : (
                                <ResponsiveContainer width="100%" height={160}>
                                    <BarChart
                                        data={asset_monitoring?.by_category ?? []}
                                        layout="vertical"
