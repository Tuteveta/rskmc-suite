        ...
        <AppLayout breadcrumbs={[{ title: 'Backup & Restore', href: '/backup' }]}>
            ...
            <div className="max-w-3xl space-y-6 p-6">
                    ...
                    <div className="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">
                                {backups.length} backup{backups.length !== 1 ? 's' : ''} stored locally
                            </p>
                        </div>
                    </div>

                    {backups.length === 0 ? (
                        <p className="py-6 text-center text-sm text-gray-400">No backups yet. Click "Create Backup" to generate one.</p>
                    ) : (
                ...
                <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-800">
                    <strong>Note:</strong> Backups are stored in <code className="rounded bg-yellow-100 px-1">storage/app/backups/</code>. Ensure{' '}
                    <code>mysqldump</code> is available on the server. Download and store copies offsite regularly.
                </div>
            </div>
        </AppLayout>
    );
